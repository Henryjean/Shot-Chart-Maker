...
calculate_hexbins_from_shots = function(shots, league_averages, binwidths = c(1, 1), min_radius_factor = 0.55, fg_diff_limits = c(-0.15, 0.15), fg_pct_limits = c(0.2, 0.7), pps_limits = c(0.5, 1.5)) {
  if (nrow(shots) == 0) {
    return(list())
  }
...
generate_hex_chart = function(hex_data, base_court, court_theme = court_themes$light, metric = sym(bounded_fg_diff), alpha_range = c(0.85, 0.98)) {
  ...
  base_court +
    ...
    scale_fill_viridis_c("FG% vs. League Avg",
                         limit = hex_data$fg_diff_limits,
...
get_data <- function(id, seasons) {
  
  df <- fetch_shots_by_player_id_and_season(id, seasons, "Regular Season")
  ...
  hex_data <- calculate_hexbins_from_shots(shots, league_averages,binwidths = c(1, 1), min_radius_factor = .55, fg_diff_limits = c(-0.15, 0.15), fg_pct_limits = c(0.2, 0.7), pps_limits = c(0.5, 1.5))  
